...
def apply_filter(image, filter_type):
    ...
    if filter_type=="red_tint":
        filtered_image[:,:, 1] = 0 # Green channel to
        filtered_image[:,:, 0] = 0 # Blue channel to
    elif filter_type == "green_tint":
        filtered_image[:, :, 0] = 0 # Blue channel to
        filtered_image[:,:, 2] = 0   # Red channel to
        ...
        combined_sobel = cv2.bitwise_or (sobelx.astype('uint8'),
                                         sobely.astype('uint8'))
...
# Load the image
image_path = 'apple.jpeg' # Provide your image path
...
if image is None:
    ...
    while True:
        filtered_image = apply_filter(image, filter_type)  # Apply the selected filter
        cv2.imshow("Filtered Image", filtered_image) # Display the filtered image
